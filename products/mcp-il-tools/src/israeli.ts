...
export interface PhoneResult {
  ...
  // 1-800 is toll-free in Israel and 1-900 is premium-rate: the caller pays
  // nothing for one and a lot for the other. Reporting both as "premium" told
  // the caller a free number would cost them money.
  type?: "mobile" | "landline" | "voip" | "toll_free" | "national" | "premium";
...
export function validateIsraeliPhone(raw: unknown): PhoneResult {
  ...
  else if (/^1800\d{6}$/.test(digits)) type = "toll_free";
  else if (/^1700\d{6}$/.test(digits)) type = "national";
  else if (/^1900\d{6}$/.test(digits)) type = "premium";
  if (!type) return { input, valid: false, reason: "unknown Israeli number format" };

  if (type === "toll_free" || type === "national" || type === "premium") {
