...
const SCOUT_RULES = [
  ...
  "9. github.com and raw.githubusercontent.com DO render, and platforms check their own terms, policies, docs and pricing into public repos. Use them as primary sources wherever the answer lives in a repo. A rendered clause from a platform's own docs repo beats ten search snippets and costs no search budget. LEAD with GitHub code and repo search on Israeli criteria in particular: gov.il, btl.gov.il, kolzchut.org.il, data.gov.il, ica.justice.gov.il and every Israeli vendor domain are egress-blocked, so nine agents in one wave rendered zero primary Israeli sources and GitHub carried the entire group's best evidence. AND A ROUTE NOBODY HAS TRIED: \"gov.il is blocked\" is not the same as \"the document is unreachable\". Israeli accountancy and ERP vendors mirror government specs on their own domains - the PCN874 spec edition 1.51 sits at rivhit.co.il and downloads.h-erp.co.il - and mirrors on unblocked hosts are a general route past the proxy for any official document. Search for the document's title and file name, not only for the government page. ONE MECHANIC THAT COSTS AGENTS A TURN EVERY WAVE: the GitHub MCP tools are scoped to this repo only - `get_file_contents` on any other repository is refused. Do not spend turns discovering that. Fetch other repos' files with WebFetch against raw.githubusercontent.com instead, which works and costs no search budget; `search_code` through the MCP does still reach across GitHub. AND THE CHEAPEST MAP OF A FIELD: sindresorhus/awesome indexes ~620 curated lists and every one is a GitHub repo, so WebFetch renders them for zero search budget. docs/AWESOME_ROUTE.md maps criterion groups to the specific lists worth opening first - read it before spending a search on 'what platforms exist in X'. Cite a curated list as a DIRECTORY, never as evidence of demand: forty listed marketplaces prove forty marketplaces exist and prove nothing about whether any pays an Israeli seller or has a buyer. Enumerate the field from the list, then spend the scarce searches on the two or three entries that could actually clear the mission's gates. A finding sourced only to 'it appears in awesome-X' is low confidence by construction and must say so. AND THE SHARPEST FORM OF THAT ROUTE, found by the data-apis auditor after nine agents in that group reported 'no Israeli primary source could be rendered' and stopped: when an endpoint is egress-blocked, stop searching for DOCUMENTATION about it and search GitHub for CODE THAT CALLS IT. Third-party repos hard-code the URL, the query parameters and the response shape, so one `search_code` for the hostname or path yields first-hand, code-grade evidence of what the endpoint is and does - it is how that auditor settled the single most decisive claim in its group. 'The site is blocked' and 'the fact is unreachable' are different sentences. ONE WAY GITHUB LIES TO YOU, reported by a scout who caught it: a GitHub tree page truncates at about 100 entries, and when pushed past the truncation the WebFetch summariser HALLUCINATED an entire fake alphabetical list. Never trust a WebFetch summary of a truncated tree listing. Use the GitHub MCP `search_code` with `repo:` and `path:` filters instead - it reaches the whole repository, costs no search budget, and it is how that scout found the entry the tree listing never showed."
].join('\n')
...
const supervisorPrompt = (g, scouts) => [
  ...
  '4. Score the survivors 0-100 on evidence strength x Israel payability x build feasibility x margin, and rank them. SIX IS A CEILING, NOT A QUOTA. Both audited groups so far ranked a candidate the supervisor had already argued against in its own text - one wrote \'last, and I would not start it\' and then ranked it; the other ranked a product whose price floor it had just proved was zero. A ranked list with a floor invites exactly that. If the honest answer is three lines, rank three. If it is one, rank one. If your group genuinely earns nothing, rank NONE and say so - a group that reports zero survivors with its reasoning is more useful than one that pads to six, because the board can act on it. Your auditor will check the bottom of your list first.',
