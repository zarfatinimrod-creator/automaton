...
export const CANDIDATE_RAILS: CandidateRail[] = [
  ...
  },
  {
    ...
    beforeUse:
      "MEASURED 7.9.2026 (research/measurements/freemius-rail.md): PAYS ISRAEL = UNKNOWN. The earlier claim that Freemius 'pays out in ILS' is REFUTED from its own code — class-fs-payment.php, CheckoutPopupOptions.ts and Pricing.js all place ILS on the buyer side, and no public file describes a payout currency; the only payout string it ships ('Payouts are in USD ... via PayPal') is in the AFFILIATE form and must not be quoted as seller terms. GITHUB CODE SEARCH was exhausted (11 searches, 14 files rendered) and freemius.com, web.archive.org and archive.org are all egress-blocked here. Exactly one page settles it: https://freemius.com/help/documentation/selling-with-freemius/supported-countries/ — render it from a host with egress (a GitHub Actions job, like pcn874-spec-watch.yml), search it for 'Israel', and record the payout methods and currency shown. Until then this rail is unusable and railConcentration() stays 'concentrated'. Also flagged, not resolved: an Israeli seller's 'sole customer' under the EULA is Freemius, Inc. (US), which cuts against zero-rating.",
    evidence: "snippet",
    source: "research/measurements/freemius-rail.md; research/colony-sweep/audits/productized-services.md §5.3",
  },
];
